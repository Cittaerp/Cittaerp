    ...
    public class InventController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var bglist = from bh in db.IV_002_INVTY
                         join bg1 in db.GB_999_MSG
                         on new { a2 = bh.transaction_type, a1 = "TRT" } equals new { a2 = bg1.code_msg, a1 = bg1.type_msg}
                         join bn in db.GB_001_EMP
                         on new { b3 = bh.manual_reference } equals new { b3 = bn.employee_code}
                        
                         select new vw_genlay
                         {
                             vwint0 = bh.inventory_sequence_number,
                             vwstring1 = bg1.name1_msg,
                             vwstring2 = bn.name,
        ...
        public ActionResult EditHeader()
        {
            ...
            gdoc = db.IV_002_INVTY.Find(Convert.ToInt16(glay.vwstring9));
            if (gdoc != null)
                read_header();
            select_query_head();
        ...
        public ActionResult EditDetails(int key1, int key2 )
        {
            ...
            IV_002_IVTYD = db.IV_002_IVTYD.Find(key1, key2);
            if (IV_002_IVTYD == null)
            {
                return View(glay);
            }
            Session["ivnt"] = IV_002_IVTYD.inventory_sequence_number;
            Session["line_num"] = IV_002_IVTYD.sequence_number;
        ...
        private void update_record()
        {
            ...
            if (cr_flag.IndexOf("Header") > 0)
            {
                ...
                {
                    glay.vwint0 = Convert.ToInt16(glay.vwstring9);
                    gdoc = db.IV_002_INVTY.Find(glay.vwint0);
                }
                gdoc.transaction_type = glay.vwstrarray0[0];
                gdoc.manual_reference = glay.vwstrarray0[1];
                gdoc.description = glay.vwstrarray0[2];
                gdoc.transaction_date = util.date_yyyymmdd(glay.vwstrarray0[3]);
                gdoc.period = glay.vwstrarray0[4];
                gdoc.year = glay.vwstrarray0[5];
            ...
            else
            {
                    ...
                    IV_002_IVTYD = db.IV_002_IVTYD.Find(glay.vwint0, Convert.ToInt16(glay.vwdecimal4));
            }

                // the detailsub button has been clicked
                glay.vwint0 = Convert.ToInt16(glay.vwstring9);
                IV_002_IVTYD.inventory_sequence_number = glay.vwint0;
                IV_002_IVTYD.sequence_number = Convert.ToInt16(glay.vwdecimal4);
                ...
                IV_002_IVTYD.reason_code = glay.vwstring0;
                IV_002_IVTYD.warehouse_from = string.IsNullOrWhiteSpace(glay.vwstring4)? "" : glay.vwstring4;
                IV_002_IVTYD.bin_location_from = string.IsNullOrWhiteSpace(glay.vwstring5)? "": glay.vwstring5;
                //IV_002_IVTYD.warehouse_to = string.IsNullOrWhiteSpace(glay.vwstring6)?"":glay.vwstring6;
                IV_002_IVTYD.bin_location_to = string.IsNullOrWhiteSpace(glay.vwstring7)?"":glay.vwstring7;
                IV_002_IVTYD.reference_number = string.IsNullOrWhiteSpace(glay.vwstring10) ? "" : glay.vwstring10;
            ...
            if (cr_flag == "CreateHeader")
            {
                ...
                //qlay.query1 = gdoc.journal_type;
                //qlay.query0 = gdoc.manual_reference_number;

                //qlay.intquery1 = gdoc.number_of_cycle;
                //qlay.intquery0 = gdoc.cycle_interval;
                //qlay.query3 = gdoc.period;
                //qlay.query4 = gdoc.year;
                //qlay.query5 = gdoc.batch_description;
        ...
        private void select_query_head(string type1 = "D")
        {
            ...
            ViewBag.emp = util.para_selectquery("62", glay.vwstrarray0[1]);
           //ViewBag.emp = new SelectList(bg2.ToList(), "query0", "query1", glay.vwstrarray0[1]);
            ...
            if (type1 == "D")
            {
               ...
               // ViewBag.ware = new SelectList(bg2.ToList(), "query0", "query1", glay.vwstring4);
                ...
                //var wre = from pf in db.IV_001_WAREH
                //          where pf.active_status == "N"
                //          orderby pf.warehouse_name
                //          select pf;
                //ViewBag.ware = new SelectList(wre.ToList(), "warehouse_code", "warehouse_name", glay.vwstring4);
                ViewBag.ware2 = util.para_selectquery("57", glay.vwstring6);
                //ViewBag.ware2 = new SelectList(bg2.ToList(), "query0", "query1", glay.vwstring6);

                ViewBag.bin = util.para_selectquery("02", glay.vwstring5,"N");
                //var pcode1 = from bg in db.GB_001_PCODE
                //             where bg.parameter_type == "02" && bg.active_status == "N"
                //             orderby bg.parameter_name
                //             select bg;
                //ViewBag.bin = new SelectList(pcode1.ToList(), "parameter_code", "parameter_name", glay.vwstring5);

                ViewBag.bin2 = util.para_selectquery("02", glay.vwstring7,"N");
                //var pcode2 = from bg in db.GB_001_PCODE
                //             where bg.parameter_type == "02" && bg.active_status == "N"
                //             orderby bg.parameter_name
                //             select bg;
                //ViewBag.bin2 = new SelectList(pcode2.ToList(), "parameter_code", "parameter_name", glay.vwstring7);
            }
        }
        ...
        private void read_details()
        {
            ...
            var bglist = from bh in db.IV_002_IVTYD
                         ...
                         select new vw_genlay
                         {
                             ...
                             vwstring5 = bh.bin_location_from,
                             //vwstring6 = bh.warehouse_to,
                             vwstring7 = bh.bin_location_to,
                         };
        ...
        private void read_header()
        {
            ...
            if (bg2list != null)
            {
                ...
                glayhead.vwstrarray1[2] = bg2list.bh.description;
                glayhead.vwstrarray1[0] = bg2list.bf2.name1_msg;
                glayhead.vwstrarray1[3] = util.date_slash(bg2list.bh.transaction_date);
                glayhead.vwstrarray1[4] = bg2list.bh.period;
                glayhead.vwstrarray1[1] = bg2list.bg2.name;

                glay.vwstrarray0[0] = bg2list.bh.transaction_type;
                glay.vwstrarray0[1] = bg2list.bh.manual_reference;
                glay.vwstrarray0[2] = bg2list.bh.description;
        ...
        {
               int key1 = Convert.ToInt16(Session["invt"]);
               ...
               glay.vwstring2 = bgdtl.description;
               glay.vwdclarray0[0] = bgdtl.quantity;
               glay.vwdecimal1 = bgdtl.unit_cost;
               glay.vwdclarray0[1] = bgdtl.extended_cost;
               glay.vwstring0 = bgdtl.reason_code;
               glay.vwstring4 = bgdtl.warehouse_from;
               glay.vwstring5 = bgdtl.bin_location_to;
               //glay.vwstring6 = bgdtl.warehouse_to;
               glay.vwstring7 = bgdtl.bin_location_to;
               glay.vwstring10 = bgdtl.reference_number;
