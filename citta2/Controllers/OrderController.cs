...
namespace CittaErp.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        public ActionResult EditDetails(int key1, int key2)
        {
            ...
            AR_002_SODT = db.AR_002_SODT.Find( qheader.intquery0, key2,0);
            if (AR_002_SODT == null)
                return View(glay);
        ...
        {
            AR_002_SODT = db.AR_002_SODT.Find(glay.vwint0, glay.vwint1,0);
            if (AR_002_SODT != null)
            {
        ...
        //public ActionResult EditConfirm(int key1, int key2)
        //{
        //    ViewBag.action_flag = "EditDetails";
        //    pubsess = (pubsess)Session["pubsess"];
        //    qheader = (queryhead)Session["qheader"];
        ...
        //[HttpPost]
        //public ActionResult EditConfirm(vw_genlay glay_in, string headtype)
        //{
        //    pubsess = (pubsess)Session["pubsess"];
        //    qheader = (queryhead)Session["qheader"];
        ...
        private void order_update(int key1, string key2, string headtype)
        {
            ...
            str1 += " quote_date vwstring0, quote_expiration_date vwstring1 from AR_002_SALES where quote_reference=" + util.sqlquote(key2);
            var str2 = db.Database.SqlQuery<vw_genlay>(str1).FirstOrDefault();

            string str = "update AR_002_SALES set quote_reference=" + util.sqlquote(key2) + ",  order_reference="+ util.sqlquote(preseq)+", order_total_ext_price=" + str2.vwdecimal0 + ", order_total_inv_tax =";
           ...
           // int.TryParse(salseqno, out hbug);
            ...
            foreach (var item in xodr)
            {
                ...
                AR_002_SODT.dis_flag = item.vwstring3;
            ...
            if (xodr.Count > 0)
            {
                if (headtype == "with")
                {
                    ...
                    createtrans();
                    string strq = "update AR_002_SODT set tax_invoiceamt1 = tax_amount1, tax_invoiceamt2 = tax_amount2, tax_amount1 = 0, tax_amount2 = 0 where sale_sequence_number =" + qheader.intquery0.ToString() + " and item_code = 'InvoiceTax'";
        ...
        private void update_record()
        {
            ...
            if (cr_flag.IndexOf("Header") > 0)
            {
                ...
                gdoc.cust_order_number = string.IsNullOrWhiteSpace(glay.vwstrarray0[10]) ? "" : (glay.vwstrarray0[10]);
            ...
            else
            {
                ...
                {
                    AR_002_SODT = db.AR_002_SODT.Find(qheader.intquery0, glay.vwint2,0);
                }
                ...
                AR_002_SODT.sale_sequence_number = qheader.intquery0;
                AR_002_SODT.line_sequence = glay.vwint2;
                AR_002_SODT.sub_line_sequence = 0;
                AR_002_SODT.sales_order_sequence = qheader.query3; // string.IsNullOrWhiteSpace(glay.vwstring7) ? "" : (glay.vwstring7);
                AR_002_SODT.item_code = string.IsNullOrWhiteSpace(glay.vwstring0) ? "" : glay.vwstring0;
                AR_002_SODT.customer_code = qheader.query1;
                AR_002_SODT.item_warehouse_code = string.IsNullOrWhiteSpace(glay.vwstring1) ? "" : glay.vwstring1;
                AR_002_SODT.quote_qty = glay.vwdclarray0[1];
                AR_002_SODT.price = glay.vwdclarray0[0];
                AR_002_SODT.tax_amount = tax;
                AR_002_SODT.price_rate = decarray[8];
                AR_002_SODT.tax_amount1 = decarray[ 2];
                AR_002_SODT.tax_amount2 = decarray[3];
                AR_002_SODT.tax_amount3 = decarray[4];
                AR_002_SODT.tax_amount4 = decarray[5];
                AR_002_SODT.tax_amount5 = decarray[6];
                AR_002_SODT.base_amount = 0;
                AR_002_SODT.ext_price = net_amt;
                ...
                AR_002_SODT.quote_amount = decarray[7];
                AR_002_SODT.actual_discount = 0;
                glay.vwdecimal0 = qheader.dquery0;
                AR_002_SODT.base_amount = AR_002_SODT.quote_amount;

                AR_002_SODT.dis_flag = "N";
                AR_002_SODT.include_tax = "";
                AR_002_SODT.analysis_code1 = "";
                AR_002_SODT.analysis_code2 = "";
                AR_002_SODT.analysis_code3 = "";
      ...
      private void waredefault()
        ...
        {
            string sqlstr = "select isnull(max(line_sequence),0) vwint1 from AR_002_SODT where line_sequence <> 999 and sales_order_sequence=" + util.sqlquote(qheader.query3);
        ...
        private void createtrans()
        {
            ...
            for (int rctr = 0; rctr < recarray.GetLength(0); rctr++)
            {
                ...
                string str1 = "insert into AR_002_SODT(sale_sequence_number,line_sequence,sub_line_sequence,sales_order_sequence,item_code,quote_qty,price,ext_price,discount_percent,discount_amount,";
            ...
            if (cr_flag.IndexOf("Details") > 0)
            {
                ...
                if (headtype == "without"||headtype=="with") //----
                {
                    ...
                    string contrl_check = (from bg in db.AR_002_SALES
                                           where bg.sale_sequence_number == qheader.intquery0
        ...
        private void select_query_head(string type1)
        {
            if (type1 == "H")
            {
                ...
                var empz = from pf in db.AR_001_STRAN
                           where pf.order == "Y" && pf.active_status == "N" && pf.quote=="N"
        ...
        public ActionResult delete_detail(string id)
        {
            ...
            db.Database.ExecuteSqlCommand(sqlstr);
            sqlstr = "select count(0) intquery0 from AR_002_SODT where dis_flag='N' and sale_sequence_number=" + qheader.intquery0.ToString();
            var delctr = db.Database.SqlQuery<querylay>(sqlstr).FirstOrDefault();
            if (delctr.intquery0==0)
            {
                sqlstr = "delete from AR_002_SODT where sale_sequence_number=" + qheader.intquery0.ToString();
                db.Database.ExecuteSqlCommand(sqlstr);
                sqlstr = "update AR_002_SALES set order_total_qty=0,order_total_ext_price=0,order_total_discount=0, order_total_inv_tax=0,order_total_tax=0,order_total_amount=0 "; 
        ...
        {
            // write your query statement
            string str = "update AR_002_SALES set approval_level=100 where sale_sequence_number=" + util.sqlquote(id);
            db.Database.ExecuteSqlCommand(str);
        ...
        public ActionResult reactivate_list(string id)
        {
            ...
            int numofday = (from bg in db.AR_002_SALES
                            where bg.sale_sequence_number == intid
            ...
            string str = "update AR_002_SALES set quote_date=" + newquotedate + ", quote_expiration_date=" + strexpdate+ " where sale_sequence_number=" + util.sqlquote(id);
            db.Database.ExecuteSqlCommand(str);
        ...
        {
            // write your query statement
            string str = "update AR_002_SALES set approval_level=101 where sale_sequence_number=" + util.sqlquote(id);
            db.Database.ExecuteSqlCommand(str);
        ...
        private void read_header()
        {
            ...
            if (bg2list != null)
            {
                ...
                glay.vwstrarray0[10] = bg2list.bh.cust_order_number;
        ...
        private void display_header()
        {
            ...
            if (bg2list != null)
            {
                ...
                glayhead.vwstrarray1[3] = (bg2list.bh.order_total_ext_price * exc_rt).ToString();
        ...
        public ActionResult get_currency(string cust_code, string tdate)
        {
            ...
            var curlist = (from bg in db.MC_001_CUREN
                           ...
                           into bg1
                           from bg2 in bg1.DefaultIfEmpty()
                           where bg2.customer_code == cust_code
                           select new
                           {
        ...
        private void call_discount()
        {
            ...
            //string str = " delete AR_002_SODT where sale_sequence_number =" + qheader.intquery0.ToString() + "and dis_flag = 'I'";
            //db.Database.ExecuteSqlCommand(str);
            ...
            strq = "select order_total_discount vwdecimal0, order_total_tax vwdecimal1, order_total_qty vwdecimal2, order_total_ext_price vwdecimal3, order_total_amount vwdecimal4 from AR_002_SALES where sale_sequence_number=" + qheader.intquery0.ToString();
        ...
        private void move_detail()
        {
            ...
            var bgdtl = (from bh in db.IV_001_ITEM
                         ...
                         into bf1
                         from bf2 in bf1.DefaultIfEmpty()
                         where bh.item_code == AR_002_SODT.item_code
                         select new { bh, bf2 }).FirstOrDefault();


            glay.vwint2 = AR_002_SODT.line_sequence;
            glay.vwstring7 = AR_002_SODT.sales_order_sequence;
            glay.vwstring0 = AR_002_SODT.item_code;
            glay.vwdclarray0[0] = AR_002_SODT.price;
            glay.vwdecimal4 = (AR_002_SODT.price * exc_rt);
            glay.vwdclarray0[1] = AR_002_SODT.quote_qty;
            glay.vwdecimal2 = (AR_002_SODT.ext_price * exc_rt);
            glay.vwdclarray0[2] = (AR_002_SODT.discount_amount * exc_rt); ;
            glay.vwdclarray0[3] = AR_002_SODT.discount_percent;
            glay.vwdclarray0[4] = (AR_002_SODT.net_amount * exc_rt); ;
            glay.vwdecimal1 = (AR_002_SODT.tax_amount * exc_rt); ;
            glay.vwdecimal3 = AR_002_SODT.base_amount;
            glay.vwdecimal5 = (AR_002_SODT.actual_discount * exc_rt); ;
            glay.vwdclarray0[5] = (AR_002_SODT.quote_amount * exc_rt); ;
            glay.vwstrarray6[0] = AR_002_SODT.analysis_code1;
            glay.vwstrarray6[1] = AR_002_SODT.analysis_code2;
            glay.vwstrarray6[2] = AR_002_SODT.analysis_code3;
            glay.vwstrarray6[3] = AR_002_SODT.analysis_code4;
            glay.vwstrarray6[4] = AR_002_SODT.analysis_code5;
            glay.vwstrarray6[5] = AR_002_SODT.analysis_code6;
            glay.vwstrarray6[6] = AR_002_SODT.analysis_code7;
            glay.vwstrarray6[7] = AR_002_SODT.analysis_code8;
            glay.vwstrarray6[8] = AR_002_SODT.analysis_code9;
            glay.vwstrarray6[9] = AR_002_SODT.analysis_code10;
            glay.vwdclarray1[0] = AR_002_SODT.tax_amount1;
            glay.vwdclarray1[1] = AR_002_SODT.tax_amount2;
            glay.vwdclarray1[2] = AR_002_SODT.tax_amount3;
            glay.vwdclarray1[3] = AR_002_SODT.tax_amount4;
            glay.vwdclarray1[4] = AR_002_SODT.tax_amount5;
            glay.vwstring5 = AR_002_SODT.discount_amount != 0 ? "Y" : AR_002_SODT.discount_percent != 0 ? "Y" : "N";
        ...
        {
            string sqlstr = " update AR_002_SALES set order_total_amount = a001, order_total_tax = a002, order_total_discount = a003, order_total_ext_price = a004, order_total_qty = a005 from AR_002_SALES ax, ";
            sqlstr += " (select sum(quote_amount) a001, sum(tax_amount) a002, sum (actual_discount) a003, sum(ext_price) a004, sum(quote_qty) as a005 from AR_002_SODT where  sales_order_sequence=" + util.sqlquote(qheader.query3);
