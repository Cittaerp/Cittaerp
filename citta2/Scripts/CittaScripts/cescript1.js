$(function () {
    ...
    $("#insertd").click(function () {
        ...
        $.ajax({
            type: "POST",
            url: URL,
            data : {op:op1, src1:src1,perd1:perd1,num1:num1,pos1:pos1,max_pos:maxp,ws_code:ws_code,operat:operat,reportid:reportid},
            error: function (xhr, status, error) {
                alert("Error: " + xhr.status + " - " + error + " - " + URL);
            },
            success: function (data) {
    ...
    $('#reportID1').change(function () {
        $("body").addClass("waiting");
        var URL = $('#loader1').data('request-daily');
        var URLext = $('#reportID1').val();
        $('#kfldr').val(URLext);
        URL = $.trim(URL);
        URLext = $.trim(URLext);
        URL = URL + "/" + URLext;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (i, state) {
                    if (state.Value == "operand" && state.Text == "operand") {
                        $('#operandID1').html(items);
                        items = "";
                    }
                    else if (state.Value == "source" && state.Text == "source") {
                        $('#sourceID1').html(items);
                        items = "";
                    }
                    else if (state.Value == "period" && state.Text == "period") {
                        $('#periodID1').html(items);
                        items = "";
                    }
                    else
                        items += "<option value='" + state.Value + "'>" + state.Text + "</option>";
    ...
    $('#operandID1').change(function () {
        $("body").addClass("waiting");
        var st = $('#operandID1').val();
        $('#kop').val(st);
        $('#ksrc').val("");
        $('#kperd').val("");
    ...
    $("#sourceID1").change(function () {
        var st1 = $('#sourceID1').val();
        $('#ksrc').val(st1);
    });

    $('#periodID1').change(function () {
        var st2 = $('#periodID1').val();
        $('#kperd').val(st2);
    });
